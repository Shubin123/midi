...
- **Pulse waves** are not a native `OscillatorNode` type. They are produced by
  ...
  step positioned by the pulse-width control — the classic trick for getting a
...
- Adjustable UI scale (75–150%), driven by a `--font-scale-factor` variable that
  the whole stylesheet sizes against.
...
created lazily on the first interaction and resumed if it is suspended. Web MIDI
additionally requires a secure context — `https://` or `http://localhost`.
